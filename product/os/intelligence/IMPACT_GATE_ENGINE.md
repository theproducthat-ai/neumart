# Nuemart Product OS — Impact Gate Engine

**Version:** 1.1
**Date:** 2026-06-24
**Status:** Active
**Renamed from:** `IMPACT_ENGINE.md` (V1.0, 2026-06-22)

---

## Purpose

The Impact Gate Engine runs **stage-gate impact checks** at specific governance gates in the product lifecycle. It converts a classified request and feature object into a severity-rated, 12-category impact map. This map determines:

- Which governance gates must be cleared before the next stage
- Which Risk Objects must be created
- Which Dependency Objects must be created
- What regression test scope is required

**When this engine runs:**
| Gate | Trigger |
|---|---|
| G3 — Before PRD approval | Mandatory for any request with `schema_change`, `payment_change`, `role_change`, or `cross_module` flags |
| G4 — Before development starts | Re-run if scope has changed since G3 |
| G5 — Before QA | Verify regression test scope from this engine's output |
| G6 — Before UAT | Confirm security and payment impact findings are addressed |
| G7 — Before release | Final check that all gate requirements are satisfied |

**Contrast with `IMPACT_ANALYSIS_ENGINE.md`:**
- This engine (IMPACT_GATE_ENGINE) = **gate-level checks per request/feature** — 12 category checklist, triggers governance gates, creates Risk and Dependency objects.
- IMPACT_ANALYSIS_ENGINE = **strategic blast-radius analysis** — cross-module dependency mapping, data flow analysis, NFR risk, and roadmap impact. Used before planning starts for complex or cross-module work.

No feature may proceed to PRD without a completed Impact Gate assessment.
...
| Module objects | `product/objects/modules/` | Yes |
...
The Impact Gate Engine evaluates 12 categories. Every category must be assessed — even if the result is `None`. A `None` rating must be explicitly assigned, not implied by absence.
...
- `IMPACT_ANALYSIS_ENGINE.md` — Upstream blast-radius analysis (run before gate checks for complex work)
- `AI_REASONING_MODEL.md` — Impact Gate Engine is Step 7 of the reasoning model
